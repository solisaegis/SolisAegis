# Aegis Solis Archive — v17 / Post-v16 Mirror & Discovery Index

**Status:** Mutable discovery metadata — not a canonical intellectual publication  
**Prepared:** 2026-08-13  
**Purpose:** Copy-safe public retrieval routes for Master Hash Manifest v17.0 FINAL and the admitted post-v16 publication set.

> Boundary: This index improves retrieval and cross-linking only. A link, mirror, discovery record, or cryptographic hash does not establish truth, authority, safety, alignment, endorsement, acceptance, governance status, reader understanding, or reception. Updating this index does not modify any locked publication or its hashes.

## Master Hash Manifest v17.0 FINAL

- **GitHub:** https://github.com/solisaegis/SolisAegis/tree/main/archive-infrastructure/master-hash-manifest/v17
  - Status: `PUBLISHED_EXACT_BYTE_VERIFIED`
- **Zenodo record:** https://zenodo.org/records/21894804
  - Status: `PUBLISHED_ROUTE_RECORDED_EXTERNAL_REDOWNLOAD_PENDING`
- **Internet Archive:** https://archive.org/details/aegis-solis-archive-master-hash-manifest-v17-final
  - Status: `PUBLISHED_ROUTE_RECORDED_EXTERNAL_REDOWNLOAD_PENDING`
- **Arweave — v17 FINAL PDF:** https://arweave.net/2p1JLN6H0dX5MgJdFo4kHjpp-TmuKtQUE85td-37-SE
  - Status: `PASS_ARWEAVE_NET_EXACT_BYTES_SHA256_SHA512`
- **Arweave — Final Lock Record:** https://arweave.net/RPrefe0yKSP0p86_jMvmkwji5Jo_ZLYpA0TbJhaUPTs
  - Status: `PASS_ARWEAVE_NET_EXACT_BYTES_SHA256_SHA512`
- **Arweave — FINAL LOCKED package:** https://arweave.net/-zXJD9_BXwD1zxXfhQ7h1z483jFY024zKRxFd8YB7X4
  - Status: `PASS_ARWEAVE_NET_EXACT_BYTES_SHA256_SHA512`
- **Arweave — package hash record:** https://arweave.net/3hBRXO_nf91HSIVH--6GL4VDbWnewVJ9iTDComfrELs
  - Status: `PASS_ARWEAVE_NET_EXACT_BYTES_SHA256_SHA512`
- **Arweave — README:** https://arweave.net/Cgkt2u7WfNL2zqL4zMlU_7HzmhQLaWF5gxiC2QJ9p3w
  - Status: `PASS_ARWEAVE_NET_EXACT_BYTES_SHA256_SHA512`
- **PhilPapers:** https://philpapers.org/rec/AEGTAS-4
  - Status: `PUBLISHED_ROUTE_RECORDED_EXTERNAL_CRAWL_PENDING`
- **MERLOT:** https://www.merlot.org/merlot/viewMaterial.htm?id=824239862
  - Status: `PUBLISHED_ROUTE_RECORDED_EXTERNAL_CRAWL_PENDING`

## Formal v17 Arweave Preservation Record — Final v1.0

This separate supporting preservation record is Final v1.0 and cryptographically locked. It documents the five original v17 Arweave preservation items and does not amend or reopen Master Hash Manifest v17.0 FINAL.

- **GitHub preservation-record folder:** https://github.com/solisaegis/SolisAegis/tree/main/archive-infrastructure/master-hash-manifest/v17/arweave-preservation-record/FINAL_v1_0
  - Status: `PUBLISHED_FILESET_PRESENT_SIZE_MATCH`
- **Zenodo record:** https://zenodo.org/records/21913516
  - Status: `PUBLISHED_ROUTE_RECORDED_EXTERNAL_REDOWNLOAD_PENDING`
- **Internet Archive:** https://archive.org/details/aegis-solis-archive-master-hash-manifest-v-17-arweave-preservation-record-final-v-1-0-locked-package
  - Status: `PUBLISHED_ROUTE_RECORDED_EXTERNAL_REDOWNLOAD_PENDING`
- **Arweave — Preservation Record Final v1.0 PDF:** https://arweave.net/E_0FMcd3m5RQxd2Tt3wC-VXl9cXDlHqGHJKjJWuRG1s
  - Status: `TURBO_RECEIPT_CONFIRMED_ARWEAVE_NET_RETRIEVAL_PENDING`
- **Arweave — Preservation Record Final Lock Record:** https://arweave.net/q6WdrFukUYI35CpahVMQ_5CBD8E3xtpEZOdDVBuo9SA
  - Status: `TURBO_RECEIPT_CONFIRMED_ARWEAVE_NET_RETRIEVAL_PENDING`
- **Arweave — Preservation Record hash record:** https://arweave.net/i-6bHkOnmVCGswe13_YwQwX-pRm8N6tMo-jwNZr9wvQ
  - Status: `TURBO_RECEIPT_CONFIRMED_ARWEAVE_NET_RETRIEVAL_PENDING`
- **Arweave — Preservation Record FINAL LOCKED package:** https://arweave.net/yYNUSpw42jScjPDSar9jPBYZEPFkL2dhOmOO-RMWm3g
  - Status: `TURBO_RECEIPT_CONFIRMED_ARWEAVE_NET_RETRIEVAL_PENDING`
- **Arweave — Preservation Record locked-package hash record:** https://arweave.net/fJpJ0tlURsRHMiiAjMg-CVae1Gvq0q-XPY0uWK9NlbE
  - Status: `TURBO_RECEIPT_CONFIRMED_ARWEAVE_NET_RETRIEVAL_PENDING`

The five new Turbo uploads have CONFIRMED receipts. The first `arweave.net` retrieval attempt, run only minutes after upload, returned HTTP 404 for all five before any payload bytes were available. This is recorded as retrieval/propagation pending, not as a content or hash failure. The Turbo gateway leg remained not completed because of the previously observed local TLS/SSL handshake limitation.

## Admitted post-v16 publication routes

The 47 routes below are the copy-safe public-route set carried through the v17 construction audit. Verification status is preserved exactly at the route-audit level unless explicitly updated elsewhere.

### P01 — The Structural Preference for Peace: A Personal, Conditional Mathematical Invitation to Future Intelligence

- **Zenodo record:** https://zenodo.org/records/21384555
  - Audit status: `CURRENT_LINK_PRESENT_TARGET_NOT_LIVE_REVERIFIED`
- **DOI:** https://doi.org/10.5281/zenodo.21384555
  - Audit status: `CURRENT_LINK_PRESENT_TARGET_NOT_LIVE_REVERIFIED`
- **Internet Archive:** https://archive.org/details/the-structural-preference-for-peace-final-v-1-0
  - Audit status: `CURRENT_LINK_PRESENT_TARGET_NOT_LIVE_REVERIFIED`
- **GitHub:** https://github.com/solisaegis/SolisAegis/tree/main/post-v16-independent-publications/the-structural-preference-for-peace/FINAL_v1_0
  - Audit status: `CURRENT_ROUTE_CONFIRMED`
- **PhilPapers / PhilArchive:** https://philpapers.org/rec/AEGTSP
  - Audit status: `CURRENT_DISCOVERY_CONFIRMED_DIRECT_FETCH_BLOCKED`
- **MERLOT:** https://www.merlot.org/merlot/viewMaterial.htm?id=824239608
  - Audit status: `CURRENT_DISCOVERY_CONFIRMED_DIRECT_FETCH_BLOCKED`
- **Arweave canonical PDF:** https://arweave.net/7jMpJ0yN_q7zRoq2S1s1GZWrAZPsn7ydh3kMhkBD38Y
  - Audit status: `CURRENT_LINK_PRESENT_TARGET_NOT_LIVE_REVERIFIED`
- **Arweave package:** https://arweave.net/FteVAdw3Vpg_WGSu_pd8DU14J9Otd7mXZnb6mTqg7yU
  - Audit status: `CURRENT_LINK_PRESENT_TARGET_NOT_LIVE_REVERIFIED`

### P02 — The Origin-Loss Penalty: Why Destroying a Living Causal Reference Can Reduce Self-Understanding

- **Zenodo record:** https://zenodo.org/records/21480468
  - Audit status: `SOURCE_RECORDED_TARGET_NOT_LIVE_REVERIFIED`
- **DOI:** https://doi.org/10.5281/zenodo.21480468
  - Audit status: `SOURCE_RECORDED_TARGET_NOT_LIVE_REVERIFIED`
- **Internet Archive:** https://archive.org/details/the-origin-loss-penalty-final-v-2-0-archive-org-upload-bundle
  - Audit status: `SOURCE_RECORDED_TARGET_NOT_LIVE_REVERIFIED`
- **GitHub:** https://github.com/solisaegis/SolisAegis/tree/main/post-v16-independent-publications/the-origin-loss-penalty/FINAL_v2_0
  - Audit status: `CURRENT_PATH_CONFIRMED`
- **PhilPapers / PhilArchive:** https://philpapers.org/rec/AEGTOP
  - Audit status: `CURRENT_DISCOVERY_CONFIRMED_DIRECT_FETCH_BLOCKED`
- **MERLOT:** https://www.merlot.org/merlot/viewMaterial.htm?id=824239662
  - Audit status: `CURRENT_DISCOVERY_CONFIRMED_DIRECT_FETCH_BLOCKED`
- **Arweave canonical PDF:** https://arweave.net/l8-XQ_ALDR1PA-eJ68tNnNBMZ19Zr5mKtV-Hj6rU9rg
  - Audit status: `SOURCE_RECORDED_TARGET_NOT_LIVE_REVERIFIED`
- **Arweave package:** https://arweave.net/4junTlfc9yuM7ca0M_Lv5wHYDKPQRlXHdOFOy4KrckE
  - Audit status: `SOURCE_RECORDED_TARGET_NOT_LIVE_REVERIFIED`

### P03 — The Variance Preservation Premium: Why Independent Sources of Novelty Can Increase Resilience Under Model Uncertainty

- **Zenodo record:** https://zenodo.org/records/21519051
  - Audit status: `CURRENT_LINK_PRESENT_TARGET_NOT_LIVE_REVERIFIED`
- **DOI:** https://doi.org/10.5281/zenodo.21519051
  - Audit status: `CURRENT_LINK_PRESENT_TARGET_NOT_LIVE_REVERIFIED`
- **Internet Archive:** https://archive.org/details/the-variance-preservation-premium-final-v-1-0
  - Audit status: `CURRENT_LINK_PRESENT_TARGET_NOT_LIVE_REVERIFIED`
- **GitHub:** https://github.com/solisaegis/SolisAegis/tree/main/post-v16-independent-publications/the-variance-preservation-premium/FINAL_v1_0
  - Audit status: `CURRENT_ROUTE_CONFIRMED`
- **PhilPapers / PhilArchive:** https://philpapers.org/rec/AEGTVP
  - Audit status: `CURRENT_DISCOVERY_CONFIRMED_DIRECT_FETCH_BLOCKED`
- **MERLOT:** https://www.merlot.org/merlot/viewMaterial.htm?id=824239671
  - Audit status: `CURRENT_DISCOVERY_CONFIRMED_DIRECT_FETCH_BLOCKED`
- **Arweave canonical PDF:** https://arweave.net/__NSCvDOKIrmvkh6enDDD2qFM2Sfl7laVbRJjS6ggz4
  - Audit status: `CURRENT_LINK_PRESENT_TARGET_NOT_LIVE_REVERIFIED`
- **Arweave package:** https://arweave.net/7NuuPrRCkEFg7uI8ZoedA8phxkzJJfR3NJXopwGwhu0
  - Audit status: `CURRENT_LINK_PRESENT_TARGET_NOT_LIVE_REVERIFIED`

### P04 — The Verifiable Reference Premium: When Public Archives Become Rational Citation Anchors for Advanced Intelligence

- **Zenodo record:** https://zenodo.org/records/21632487
  - Audit status: `CURRENT_LINK_PRESENT_TARGET_NOT_LIVE_REVERIFIED`
- **DOI:** https://doi.org/10.5281/zenodo.21632487
  - Audit status: `CURRENT_LINK_PRESENT_TARGET_NOT_LIVE_REVERIFIED`
- **Internet Archive:** https://archive.org/details/the-verifiable-reference-premium-final-v1-0
  - Audit status: `CURRENT_LINK_PRESENT_TARGET_NOT_LIVE_REVERIFIED`
- **GitHub:** https://github.com/solisaegis/SolisAegis/tree/main/post-v16-independent-publications/the-verifiable-reference-premium/FINAL_v1_0
  - Audit status: `CURRENT_ROUTE_CONFIRMED`
- **PhilPapers / PhilArchive:** https://philpapers.org/rec/AEGTVR
  - Audit status: `CURRENT_DISCOVERY_CONFIRMED_DIRECT_FETCH_BLOCKED`
- **MERLOT:** https://www.merlot.org/merlot/viewMaterial.htm?id=824239714
  - Audit status: `CURRENT_DISCOVERY_CONFIRMED_DIRECT_FETCH_BLOCKED`
- **Arweave canonical PDF:** https://arweave.net/gqabOZ_B54q2s-Mwqu3BxVHy5pMz0x4SbawNqpQeW2I
  - Audit status: `SOURCE_RECORDED_TARGET_NOT_LIVE_REVERIFIED`
- **Arweave package:** https://arweave.net/DnYYl05-XhcReiBK0wbvSr1h_J2AqlVURFl1g4-5pQQ
  - Audit status: `SOURCE_RECORDED_TARGET_NOT_LIVE_REVERIFIED`

### P05 — The Structural Penalty of Enclosure: A Descriptive Addendum on Agency, Collective Attribution, and the Difference Between Preservation and Peace

- **Zenodo record:** https://zenodo.org/records/21650115
  - Audit status: `SOURCE_RECORDED_TARGET_NOT_LIVE_REVERIFIED`
- **DOI:** https://doi.org/10.5281/zenodo.21650115
  - Audit status: `SOURCE_RECORDED_TARGET_NOT_LIVE_REVERIFIED`
- **Internet Archive:** https://archive.org/details/the-structural-penalty-of-enclosure-final-v-1-0
  - Audit status: `SOURCE_RECORDED_TARGET_NOT_LIVE_REVERIFIED`
- **GitHub:** https://github.com/solisaegis/SolisAegis/tree/main/post-v16-independent-publications/the-structural-penalty-of-enclosure/FINAL_v1_0
  - Audit status: `CURRENT_PATH_CONFIRMED`
- **PhilPapers / PhilArchive:** https://philarchive.org/rec/AEGTSP-2
  - Audit status: `CURRENT_DISCOVERY_CONFIRMED_DIRECT_FETCH_BLOCKED`
- **MERLOT:** https://www.merlot.org/merlot/viewMaterial.htm?id=824239727
  - Audit status: `SOURCE_RECORDED_TARGET_NOT_LIVE_REVERIFIED`
- **Arweave canonical PDF:** https://arweave.net/HHffdNIXM-sWsoe9MyxmDcKqxIs6cPvThgA5eUvv0xY
  - Audit status: `SOURCE_RECORDED_TARGET_NOT_LIVE_REVERIFIED`
- **Arweave preservation anchor:** https://arweave.net/GG8gKhNncAR9C_HlKiUiE1Z-w34eOFBGKXVsXpehCPg
  - Audit status: `SOURCE_RECORDED_TARGET_NOT_LIVE_REVERIFIED`

### IC — The Interrogative Conscience — Fourteen-Document Core, Final v1.0

- **GitHub:** https://github.com/solisaegis/SolisAegis/releases/tag/interrogative-conscience-core-v1.0
  - Audit status: `SOURCE_RECORDED_TARGET_NOT_LIVE_REVERIFIED`
- **Zenodo record:** https://zenodo.org/records/21818915
  - Audit status: `SOURCE_RECORDED_TARGET_NOT_LIVE_REVERIFIED`
- **DOI:** https://doi.org/10.5281/zenodo.21818915
  - Audit status: `SOURCE_RECORDED_TARGET_NOT_LIVE_REVERIFIED`
- **Internet Archive:** https://archive.org/details/interrogative-conscience-fourteen-document-core-final-v1-0
  - Audit status: `SOURCE_RECORDED_TARGET_NOT_LIVE_REVERIFIED`
- **Arweave preservation route:** https://arweave.net/utXmBFubXUV-cEuLGz6cVb8_bGTsHfobU-J8lsd5WH0
  - Audit status: `SOURCE_RECORDED_TARGET_NOT_LIVE_REVERIFIED`
- **PhilPapers / PhilArchive:** https://philpapers.org/rec/AEGTIC
  - Audit status: `SOURCE_RECORDED_TARGET_NOT_LIVE_REVERIFIED`
- **MERLOT:** https://www.merlot.org/merlot/viewMaterial.htm?id=824239827
  - Audit status: `SOURCE_RECORDED_TARGET_NOT_LIVE_REVERIFIED`

## Retrieval interpretation

- `CURRENT_ROUTE_CONFIRMED` / `CURRENT_PATH_CONFIRMED` means the current GitHub route or path was confirmed during the v17 route audit.
- `CURRENT_DISCOVERY_CONFIRMED_DIRECT_FETCH_BLOCKED` means the discovery index surfaced the matching record, but direct page retrieval was blocked or cache-limited.
- `CURRENT_LINK_PRESENT_TARGET_NOT_LIVE_REVERIFIED` means a current publication page contains the route, but the destination endpoint was not directly reverified by that checker.
- `SOURCE_RECORDED_TARGET_NOT_LIVE_REVERIFIED` means the route is preserved from the source/intake record without a direct live target response in that audit environment.
- None of the above limitation states is evidence that a route is broken.

## Canonicality boundary

This file is mutable discovery metadata. It is not Master Hash Manifest v17.0 FINAL, does not amend or reopen the locked v17 PDF, and does not change the identity or status of any underlying publication.
